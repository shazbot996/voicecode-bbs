...
- The `publish/` module contains document publishing agents (ARCH, PLAN, SPEC, GLOSSARY, CONSTRAINTS) — each subclasses `PublishAgent` with a prompt template that gets sent through the normal agent execution pipeline. GLOSSARY and CONSTRAINTS use fixed destination folders (`context/`). The publish overlay UI is in `ui/publish_overlay.py`.
