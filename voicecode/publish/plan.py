"""Implementation plan publishing agent."""

from voicecode.publish.base import PublishAgent


class PlanAgent(PublishAgent):
    doc_type = "PLAN"
...
You will be given a build specification (scope) describing a feature, change, or \
piece of work to implement. Your job is to analyze the spec together with the \
existing codebase, then produce a detailed, step-by-step implementation plan in \
Markdown. The plan should be concrete enough that a developer (or an AI agent) \
can follow it without guessing — every step should reference real files, real \
functions, and real patterns already in the codebase.
...
### 2. Context & Prior Art
- Relevant existing code, patterns, or conventions in the codebase that this \
plan builds on or must be consistent with.
...
- **What**: Describe the change — new file, edit to an existing file, \
configuration change, etc.
- **Where**: Exact file path(s) and, where helpful, function/class names.
- **How**: Enough detail to implement without ambiguity. Reference existing \
patterns in the codebase (e.g. "follow the same pattern as `ArchAgent` in \
`publish/arch.py`"). Include code sketches for non-obvious logic.
...
Every step should be grounded in the real codebase.
- **Respect existing patterns.** If the codebase does something a certain way, \
the plan should follow that convention unless there is a good reason not to.
- **Order matters.** Steps should be in a logical implementation order — \
foundations first, integration last.
- **Keep it actionable.** Each step should be something a developer can sit down \
and do. Avoid vague steps like "implement the feature" — break it down.
- Write the document as a single Markdown file. Use `##` for top-level sections \
and `###` for subsections.
- Start the document with a YAML frontmatter block containing title, spec \
reference, scope, and date.
"""
