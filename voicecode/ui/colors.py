...
def _resolve_color(color_val: int) -> int:
    ...
    max_colors = getattr(curses, "COLORS", 256)
    if color_val >= max_colors:
        return FALLBACK_MAP.get(color_val, curses.COLOR_WHITE)
    return color_val


def init_colors(theme_name: str = "pcboard", tts_available: bool = True):
    ...
    # TTS pair is always the theme's "tts" role (white text across themes)
    tts_def = theme.get("tts", {"fg": curses.COLOR_WHITE, "bg": -1})
...
def set_active_theme(name: str, tts_available: bool = True):
